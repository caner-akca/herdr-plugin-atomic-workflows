...
/** System-prompt leash for the task pane's Atomic session. Observed 2026-08-29:
 * after a workflow run failed, the pane's idle main-chat model autonomously
 * launched a follow-up builtin `goal` workflow on an expensive model. A task
 * pane runs exactly one managed workflow; anything further needs a human. */
export const TASK_SESSION_LEASH = [
  "This session exists to run exactly one managed workflow to completion.",
  "When that workflow finishes, succeeds, fails, or is interrupted: summarize the outcome, answer questions about it, and stop.",
  "Never start another workflow (including the builtin goal workflow), retry, repair, or modify workflow or plugin infrastructure on your own initiative.",
  "Any follow-up work requires a new explicit instruction typed by a human in this pane.",
].join(" ");
